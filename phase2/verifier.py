...
def verify_group(group_id: str, members: list, mconn, dconn,
                 gcreds, od_token: str, use_phash: bool, dry_run: bool) -> str:
    """
    ...
    if use_phash:
        photo_members = [m for m in members
                         if Path(m['filename']).suffix.lower() in PHOTO_EXTS
                         and m['source_path'] and Path(m['source_path']).exists()]
...
def main():
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"),
        ...
        for i, g in enumerate(groups):
            progress.update(task,
                description=f"[cyan]Verifying[/cyan] {g['keep_filename'][:50]}",
                completed=i)
            ...
            result = verify_group(
                g['group_id'], members, mconn, dconn,
                gcreds, od_token, args.phash, args.dry_run
            )
            stats[result] += 1
    ...
    table.add_row("✓ Confirmed (→100%)",  str(stats['confirmed']),
                  "Partial hashes match — safe to delete")
    table.add_row("✗ Rejected  (→40%)",   str(stats['rejected']),
                  "Hashes differ — removed from delete list")
    table.add_row("~ Skipped",            str(stats['skipped']),
                  "Couldn't download/access to verify")
    if args.phash:
        table.add_row("≈ pHash match (→95%)", str(stats['phash_match']),
                      "Visually identical — safe to delete")
    ...
    console.print()
    console.print(f"  [dim]Run Phase 2 classifier with --reset to regenerate reports with updated confidence scores.[/dim]")
    console.print()
    console.rule()
