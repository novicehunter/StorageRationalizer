...
PHOTO_EXTS = {'.jpg','.jpeg','.png','.gif','.heic','.heif','.tiff','.tif',
...
SKIP_DIRS  = {'.git','node_modules','.Trash','__pycache__','.cache',
...
def categorize(ext):
    ext = ext.lower()
    return (1 if ext in PHOTO_EXTS else 0,
...
def base_record(source):
    return {
        'file_id': str(uuid.uuid4()), 'source': source,
        'source_path': None, 'cloud_file_id': None,
        'filename': '', 'file_ext': '', 'file_size': None, 'mime_type': '',
        'sha256_hash': None, 'md5_hash': None, 'sha1_hash': None, 'quick_xor_hash': None,
        'created_at': None, 'modified_at': None, 'exif_date': None,
        'latitude': None, 'longitude': None, 'width': None, 'height': None,
        'is_photo': 0, 'is_video': 0, 'is_document': 0,
        'is_downloaded': 1, 'is_favorite': 0, 'is_edited': 0,
        'is_live_photo': 0, 'is_screenshot': 0, 'is_selfie': 0, 'is_portrait': 0,
        'media_type_flags': None, 'album_name': None,
        'parent_folder': None, 'drive_name': None,
        'raw_metadata': None, 'scanned_at': now_iso(), 'scan_error': None,
    }
...
def scan_local(conn, progress, task):
    ...
    for root in LOCAL_SCAN_PATHS:
        ...
        for path in root.rglob('*'):
            ...
            try:
                path.relative_to(PHOTOS_LIBRARY); continue
            except ValueError: pass
            try:
                path.relative_to(ICLOUD_DRIVE_PATH); continue
            except ValueError: pass
            ...
            try:
                ...
                r.update({
                    ...
                    'created_at':   datetime.fromtimestamp(stat.st_birthtime, timezone.utc).isoformat() if hasattr(stat,'st_birthtime') else None,
                    'modified_at':  datetime.fromtimestamp(stat.st_mtime, timezone.utc).isoformat(),
                    'is_photo': p, 'is_video': v, 'is_document': d,
                    'parent_folder': str(path.parent),
                })
                insert_file(conn, r)
                stats['new'] += 1
                if stats['new'] % 100 == 0: conn.commit()
...
def scan_icloud_photos(conn, progress, task):
    ...
    for i, photo in enumerate(photos):
        ...
        try:
            ...
            flags = []
            if getattr(photo, 'live_photo',     False): flags.append('live_photo')
            if getattr(photo, 'screenshot',     False): flags.append('screenshot')
            if getattr(photo, 'selfie',         False): flags.append('selfie')
            if getattr(photo, 'portrait',       False): flags.append('portrait')
            if getattr(photo, 'hdr',            False): flags.append('hdr')
            if getattr(photo, 'panorama',       False): flags.append('panorama')
            if getattr(photo, 'slow_mo',        False): flags.append('slow_mo')
            if getattr(photo, 'time_lapse',     False): flags.append('time_lapse')
            if getattr(photo, 'burst',          False): flags.append('burst')
            ...
            insert_file(conn, r)
            stats['new'] += 1
            if stats['new'] % 200 == 0: conn.commit()
    ...
    while True:
        try:
            resp = service.files().list(
                pageSize=1000, fields=fields, pageToken=pt, q="trashed=false",
                includeItemsFromAllDrives=True, supportsAllDrives=True, corpora='allDrives',
            ).execute()
        except Exception as e:
            log_error('google_drive', str(e)); break
        ...
        for f in resp.get('files', []):
            ...
            fid = f.get('id')
            if already_scanned(conn, 'google_drive', fid): continue
            try:
                name = f.get('name',''); ext = Path(name).suffix.lower()
                p, v, d = categorize(ext)
                im = f.get('imageMediaMetadata',{}); vm = f.get('videoMediaMetadata',{})
                r = base_record('google_drive')
                r.update({
                    'cloud_file_id': fid, 'filename': name, 'file_ext': ext,
                    'file_size': int(f['size']) if f.get('size') else None,
                    'mime_type': mime, 'md5_hash': f.get('md5Checksum'),
                ...
                stats['errors'] += 1; log_error('google_drive', f"{fid}: {e}")
...
def scan_google_photos(conn, progress, task):
    ...
    while True:
        try:
            ...
            resp = rl.get('https://photoslibrary.googleapis.com/v1/mediaItems',
            ...
            log_error('google_photos', str(e)); break

        for item in data.get('mediaItems', []):
            stats['found'] += 1
            progress.update(task, description=f"[magenta]Google Photos[/magenta] {item.get('filename','')[:55]}")
            iid = item.get('id')
            if already_scanned(conn, 'google_photos', iid): continue
            try:
                meta = item.get('mediaMetadata',{}); mime = item.get('mimeType','')
                name = item.get('filename','');       ext  = Path(name).suffix.lower()
                r = base_record('google_photos')
                r.update({
                    'cloud_file_id': iid, 'filename': name, 'file_ext': ext,
                    'mime_type': mime,
                    'created_at': meta.get('creationTime'), 'exif_date': meta.get('creationTime'),
                ...
                stats['errors'] += 1; log_error('google_photos', f"{iid}: {e}")
    ...
    def scan_folder(url, folder_path):
        while url:
            try:
                ...
                log_error('onedrive', f"{folder_path}: {e}"); return

            for item in data.get('value', []):
                if 'folder' in item:
                    scan_folder(f"{base_url}/me/drive/items/{item['id']}/children",
                                f"{folder_path}/{item['name']}")
                    continue
                stats['found'] += 1
                progress.update(task, description=f"[yellow]OneDrive[/yellow] {item.get('name','')[:55]}")
                iid = item.get('id')
                if already_scanned(conn, 'onedrive', iid): continue
                try:
                    name = item.get('name',''); ext = Path(name).suffix.lower()
                    p, v, d = categorize(ext)
                    hashes = item.get('file',{}).get('hashes',{})
                    im = item.get('photo',{}); vm = item.get('video',{}); fs = item.get('fileSystemInfo',{})
                    ...
                    r.update({
                        ...
                        'latitude': im.get('latitude'), 'longitude': im.get('longitude'),
                        'width': im.get('width') or vm.get('width'),
                        'height': im.get('height') or vm.get('height'),
                        'is_photo': p, 'is_video': v, 'is_document': d,
                        'is_downloaded': 0,
                    ...
                    stats['errors'] += 1; log_error('onedrive', f"{iid}: {e}")
...
def print_summary(conn):
    ...
    """):
        table.add_row(row['source'], str(row['total']),
                      str(row['photos'] or 0), str(row['videos'] or 0),
                      str(row['docs'] or 0), str(row['cloud_only'] or 0),
                      format_size(row['sz']))
    ...
    if row and row['t']:
        ...
        console.print(f"    Total: {row['t']:,}  |  Downloaded locally: {row['dl']:,}  |  Cloud-only: {row['t']-row['dl']:,}")
        console.print(f"    Favorites: {row['fav']}  |  Live Photos: {row['live']}  |  Selfies: {row['sf']}  |  Portraits: {row['pt']}  |  Screenshots: {row['ss']}  |  Edited: {row['ed']}")
        console.print()
...
def main():
    ...
    parser.add_argument('--sources', nargs='+',
                        choices=list(SCANNERS.keys()) + ['all'], default=['all'])
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"),
                  ...
                  TimeElapsedColumn(), console=console, transient=False) as progress:
        for key in sources:
            ...
            try:
                ...
                progress.update(task,
                    ...
                    completed=s['found'], total=max(s['found'], 1))
            except Exception as e:
